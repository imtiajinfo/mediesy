...
<div class="row my-auto align-items-center bg-white shadow-md border mb-3 py-2">
    <div class="col-sm-6">
        ...
        <ol class="breadcrumb bg-white">
            <li class="breadcrumb-employee"> <a href="{{ route('admin.dashboard') }}" class="text-primary"><i class="fa fa-home"></i> </a></li>
            <li class="breadcrumb-employee text-dark px-2"><a href="{{ route('admin.employees.index') }}"> employees</a>
            </li>
        </ol>
    </div>
    <div class="col-md-6">
        <ol class="float-right button">
            <a href="{{ route('admin.employees.create') }}" class="btn btn-success" rel="tooltip" id="add" title="add">
                Add New
            </a>
        </ol>
    </div>
        ...
        <div class="card">
            <div class="card-header justify-content-between py-3">
                ...
                <div class="float-right d-flex my-auto gap-3">
                    ...
                    <form class="mb-0 pb-0" id="sort_employees" action="" method="GET">
                ...
                <table id="example1" class="table table-hover">
                    ...
                    <tbody id="datatable">
                        ...
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td>
                                <img src="{{ asset('uploads/employee/' . $employee->image) }}" alt="{{ $employee->image }}" width="80">
                            </td>
                            <td class="employees-name text-start" style="line-height:1.5; min-width:150px">
                                <p>{{ $employee->name }}</p>
                            </td>
                            <td>{{ $employee->store->name }}</td>
                            <td>{{ $employee->role }}</td>
                            <td>{{ $employee->email }}</td>
                            <td style="width:25%">{{ $employee->phone }}</td>
                            ...
                            <td style="min-width:130px" class="d-flex my-4">
                                <a href="{{ route('admin.employees.edit', $employee->id) }}" class="btn btn-primary btn-sm edit text-light border-0 edit" rel="tooltip" id="edit" title="Edit" data-id="4">
                                    <i class="fa-regular fa-pen-to-square"></i>
                                </a>
                                <a href="{{ route('admin.employees.show', $employee->id) }}" class="btn btn-info btn-sm text-light view border-0 view" id="view" rel="tooltip" title="view">
                    ...
                    <div class="pagination d-flex justify-content-between">
                        ...
                        <div class="d-flex">
                            ...
                            <div class="dataTables_info pl-2">
                                Showing
                                <span id="showing-entries-from">{{ $employees->firstItem() }}</span>
                                to
                                <span id="showing-entries-to">{{ $employees->lastItem() }}</span>
                                of
                                <span id="total-entries">{{ $employees->total() }}</span>
                                entries
                            </div>
                        </div>
                        <!-- ... (remaining content) -->
                        {{ $employees->links('components.pagination.default') }}
