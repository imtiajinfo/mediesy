        ...
        <div class="container p-4">
            <form method="POST" action="{{ route('admin.colors.update',  $query->id) }}" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="mb-3 col-12 col-md-4 col-lg-3">
                        <div class="form-group">
                            <label for="code">Code</label><input type="color" name="code" class="form-control" value="{{ old('code', optional($query)->code) }}" placeholder="Enter code..." required>@error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                    </div>
                    <div class="mb-3 col-12 col-md-4 col-lg-3">
                        <div class="form-group">
                            <label for="description">Description</label><textarea name="description" class="form-control" required>{{ old('description', optional($query)->description) }}</textarea>@error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                    </div>
