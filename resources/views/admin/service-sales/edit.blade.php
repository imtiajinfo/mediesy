...
<div class="row my-auto align-items-center bg-white shadow-md border mb-3 py-2">
    <div class="col-sm-6">
        ...
        <ol class="breadcrumb bg-white">
            ...
            <li class="breadcrumb-item text-dark"><a href="{{ route('admin.purchase-orders.index') }}">Sell Orders</a>
            </li>
        </ol>
    </div>
</div>
...
<div class="row">
    <div class="card bg-white p-0">
        <div class="card-header justify-content-between py-3">
            <h4 class="card-title float-left pt-2">Edit Sell Order <a href="#">#{{$sell->sale_code}}</a> </h4>
        </div>
        <div class="container p-4 pt-0 pb-0">
            <form method="POST" action="{{ route('admin.service-sales.update', $sell->id) }}" enctype="multipart/form-data">
                @csrf
                @method("PUT")

                <input type="hidden" value="{{$sell->customer_id}}" name="customer_id">
                <input type="hidden" value="{{date('Y-m-d', strtotime($sell->sell_date))}}" name="sell_date">
                    ...
                    <div class="card mt-2 pt-1 mb-3 pb-3">
                        ...
                        <div class="row">
                            ...
                            <div class="mb-3 col-md-3">
                                ...
                                <input disabled type="date" class="form-control" name="sell_date" value="{{date('Y-m-d', strtotime($sell->sell_date))}}">
                            </div>
                        ...
                        <div id="tableContainer" style="max-height: 490px;overflow-y:scroll">
                            <table class="table table-sm">
                                ...
                                <tbody id="purchaseBody">
                                    @foreach ($sellItems as $key => $item)
                                        <tr>
                                            <td>
                                                <span class="productSl">{{$key+1}}</span> 
                                                <input type="hidden" name="sell_info_id[]" value="{{$item->id}}" required="">
                                                <input type="hidden" name="product_id[]" id="product_id" value="{{$item->product_id}}" required="">
                                                <input type="hidden" name="available_qty[]" value="{{$item->current_stock + $item->quantity}}">
                                                <input type="hidden" name="whole_sale_price[]" value="{{$item->whole_sale_price}}">
                                                <input type="hidden" name="whole_sale_qty[]" value="{{$item->whole_sale_qty}}">
                                                <input type="hidden" name="sell_price1[]" value="{{$item->sell_price1}}">
                                            </td> 
                                            <td style="width:47%">
                                                <input type="text" name="sub_title[]" value="{{ $item->itemInfo->name.' '.$item->itemInfo->strength.' '.$item->itemInfo->description.' '.$item->itemInfo->brand.' '.$item->itemInfo->manufacturer }}" class="form-control" disabled="">
                                            </td> 
                                            {{-- <td>
                                                <input type="text" name="sub_title[]" value="{{$item->sub_title}}" class="form-control sub_title" disabled="">
                                            </td> --}}
                                            <td>
                                                <input type="text" name="sell_price[]" value="{{$item->sell_price}}" class="form-control sell_price" onkeypress="return /[0-9.]/i.test(event.key)">
                                            </td> 
                                            <td style="width:8%"> 
                                                <input type="text" name="discount[]" value="{{$item->discount}}" class="form-control discount" onkeypress="return /[0-9.]/i.test(event.key)">
                                            </td> 
                                            <td style="width:8%" class="item_qty">
                                                
                                                <input type="text" name="sell_qty[]" onkeyup="updateTotalPurchaseQty()" class="form-control" required="" value="{{$item->quantity}}" onkeypress="return /[0-9.]/i.test(event.key)">
                                            </td>  
                                            <td class="sub_total">
                                                <input type="text" name="sub_total[]" value="{{$item->sub_total}}" class="form-control" onkeyup="updateTotalPurchaseQty()" disabled="">
                                            </td>
                                            <td>
                        ...
                        <div class="card-body mt-5 p-2">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <div class="col-md-6 mt-2">
                                        <textarea name="remarks" class="form-control" placeholder="Enter Rremarks...">{{$sell->payment_details}}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mt-2">
                                            <input disabled type="number" class="form-control form-control-sm" id="total_price" placeholder="0.00"  name="total_price" value="{{$sell->payable}}">
                                        </div>
                                        ...
                                        <div class="col-md-6 mt-2">
                                            <input type="text" class="form-control form-control-sm" id="collect_amount" placeholder="0.00" name="collect_amount"  value="{{$sell->paid}}" onkeypress="return /[0-9.]/i.test(event.key)">
                                        </div>
                                        ...
                                        <div class="col-md-6 mt-2">
                                            <input disabled type="text" class="form-control form-control-sm" id="due-total_price" placeholder="0.00" value="{{$sell->due}}">
                                        </div>
                        ...
                        <div id="save_purchase_container" class="text-right mt-4 mb-3">
                            <button id="save_purchase" class="btn btn-md btn-success">Update Sell</button>
...
<script>
    ...
    function addProduct() {
        let productId = $("#product-ajax-select2").val();
        productSl++;
