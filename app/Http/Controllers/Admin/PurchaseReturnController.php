...
class PurchaseReturnController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = ReturnMaster::with(['return_details', 'supplier', 'ression'])
            ->where('type', 1)
            ->orderBy('id', 'desc');
        ...
        return view('admin.purchase-return.index', compact('purchase_returns', 'search'));
    }
    ...
    {
        $purchase_return = PurchaseOrders::where('id',$request->invoice_no)->with(['purchaseOrderChildren', 'supplier'])->first();
        $return_category = ReturnCategory::all();
        $invoice_no = $request->invoice_no;

        return view('admin.purchase-return.create', compact('purchase_return', 'invoice_no', 'return_category'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if(!empty($return_qty)){
            ...
            $returnMaster = ReturnMaster::create([
                ...
                'cus_sup_id'    => $request->supplier_id,
                'user_id'       => Auth::id(),
                'invoice_no'    => $request->invoice_no,
            ...
            $purchaseMaster = PurchaseOrders::find($request->id);
            $total_return_qty = $purchaseMaster->total_received_qty - array_sum($request->return_qty);
            $purchaseMaster->total_received_qty = $total_return_qty;
            $purchaseMaster->save();

            DB::table('item_stock_masters')->insert([
                'status'                  => 3, //purchase return
                'stock_out'               => array_sum($request->return_qty),
            ...
            foreach ($return_qty as $key => $item) {
                ...
                $purchaseChild = PurchaseOrdersChild::find($request->child_id[$key]);
                $purchaseChild->update([
                    'return_qty' => $purchaseChild->return_qty + $request->return_qty[$key]
                ]);
                ...
                ItemStockChild::create([
                    ...
                    'stock_out'              => $request->return_qty[$key],
                    'receive_rate'           => 0,
            ...
            return redirect()->route('admin.purchase-return.index')->with('success', "Purchase Return Added successfully");
        }else{
    ...
    public function show($id){
        ...
        return view('admin.purchase-return.show', compact('returnDetails', 'returnMaster'));
    }
